  ...
  if (isReportData(data)) {
    ...
    return (
      <div data-testid="report-view" className="space-y-5">
        <section data-testid="report-needs" className="rounded-[var(--radius-2xl)] border border-[var(--border)] bg-[var(--surface-subtle)] p-4">
          <h3 className="text-sm font-bold text-[var(--text)]">我的需求</h3>
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            <div>
              <p className="text-[length:var(--text-xs)] font-bold uppercase tracking-[0.08em] text-[var(--text-muted)]">预算</p>
              <p className="mt-1 text-sm text-[var(--text-soft)]">{data.userProfile.budget}</p>
            </div>
            <div>
              <p className="text-[length:var(--text-xs)] font-bold uppercase tracking-[0.08em] text-[var(--text-muted)]">燃油偏好</p>
              <p className="mt-1 text-sm text-[var(--text-soft)]">{data.userProfile.fuelPreference}</p>
            </div>
            <div>
              <p className="text-[length:var(--text-xs)] font-bold uppercase tracking-[0.08em] text-[var(--text-muted)]">使用场景</p>
              <p className="mt-1 text-sm text-[var(--text-soft)]">{data.userProfile.useCases.join('、') || '未标注'}</p>
            </div>
            <div>
              <p className="text-[length:var(--text-xs)] font-bold uppercase tracking-[0.08em] text-[var(--text-muted)]">核心维度</p>
              <p className="mt-1 text-sm text-[var(--text-soft)]">{dimensions.join('、') || '未标注'}</p>
            </div>
          </div>
        </section>

        <section data-testid="report-comparison" className="rounded-[var(--radius-2xl)] border border-[var(--border)] bg-[var(--surface)] p-4">
          <h3 className="text-sm font-bold text-[var(--text)]">候选车对比</h3>
          <div className="mt-4 space-y-4">
            {data.comparison.map((item) => (
              <article key={item.carName} className="rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--surface-subtle)] p-4">
                <div className="flex items-center justify-between gap-3">
                  <h4 className="text-sm font-semibold text-[var(--text)]">{item.carName}</h4>
                  <p className="text-[length:var(--text-xs)] text-[var(--text-muted)]">{item.highlight}</p>
                </div>
                <div className="mt-3 space-y-2">
                  {Object.entries(item.scores).map(([label, score]) => (
                    <div key={label}>
                      <div className="mb-1 flex items-center justify-between text-[length:var(--text-xs)]">
        ...
        <section data-testid="report-recommendation" className="rounded-[var(--radius-2xl)] border border-[var(--accent-border)] bg-[var(--accent-muted)] p-4">
