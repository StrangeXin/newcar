    ...
    <section className="rounded-ws-lg border border-[var(--border)] bg-[var(--surface)] p-ws14 shadow-workspace">
      <div className="flex min-h-[28px] items-center justify-between gap-[10px]">
        <h3 className="flex items-center gap-1.5 text-[13px] font-extrabold text-[var(--text)]">
          <Sparkles className="h-4 w-4 text-[var(--accent-text)]" aria-hidden="true" />
          AI 旅程摘要
        </h3>
        <button
          type="button"
          ...
          className="inline-flex cursor-pointer items-center gap-1 rounded-ws-sm border-[1.5px] border-[var(--border)] bg-[var(--surface)] px-[10px] py-[6px] text-[10px] font-semibold leading-[1.2] text-[var(--text-soft)] hover:border-[var(--border-soft)] disabled:cursor-not-allowed"
        >
      ...
      {displaySnapshot ? (
        <div className="mt-[14px] space-y-[14px]">
          ...
          <div>
            <h4 className="flex items-center gap-1 text-[10px] font-bold text-[var(--text-soft)]">
              <Lightbulb className="h-3.5 w-3.5 text-[var(--warning-text)]" aria-hidden="true" />
              关键洞察
            </h4>
            <ul className="mt-[10px] space-y-[10px]">
              {insights.slice(0, 3).map((item) => (
                <li key={item.insight} className={`rounded-[10px] border-l-[3px] px-[10px] py-[10px] text-sm ${getInsightTone(item.confidence)}`}>
                  <p className="text-[11px] font-bold text-[var(--text)]">{item.insight}</p>
                  <p className="mt-1 text-[10px] leading-[1.4] text-[var(--text-soft)]">{item.evidence}</p>
                  <p className="mt-1 text-[9px] text-[var(--text-muted)]">置信度 {Math.round(item.confidence * 100)}%</p>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            <h4 className="flex items-center gap-1 text-[10px] font-bold text-[var(--text-soft)]">
              <ListChecks className="h-3.5 w-3.5 text-[var(--success-text)]" aria-hidden="true" />
              AI 建议下一步
            </h4>
            <div className="mt-[6px] flex flex-wrap gap-[6px]">
              {actions.slice(0, 3).map((action) => (
                <button
                  key={action}
                  type="button"
                  className="inline-flex cursor-pointer items-center justify-center rounded-full border border-[var(--border)] bg-[var(--surface-subtle)] px-[10px] py-1 text-[10px] font-medium leading-[1.2] text-[var(--text-soft)] hover:border-[var(--border)]"
                >
                  {action}
                </button>
              ))}
