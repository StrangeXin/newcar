...
|-------|------------|----------------|------|
| `--bg` | `#fff9f4` | `#f8fafc` | 页面底色 |
| `--bg-gradient` | 多径向渐变（暖橙/绿） | 无渐变，纯色 | body 背景 |
...
/* 主题 2 shadow-card: 将橙色替换为 rgba(99,102,241,0.05) */
--shadow-accent: 0 2px 8px rgba(249,115,22,0.30);
/* 主题 2: rgba(99,102,241,0.25) */
```
...
--font-heading: 'Playfair Display', 'Noto Serif SC', 'PingFang SC', serif;
...
- 页面大标题、卡片标题：`font-family: var(--font-display)`，`font-weight: 700–800`，`letter-spacing: -0.02em`
- 正文、标签、说明：`font-family: var(--font-body)`，`font-weight: 400–600`
- 英文 label（全大写）：`letter-spacing: 0.12–0.18em`，`font-size: 10–11px`
- 中文 label：不做 `text-transform: uppercase`，保持正常大小写
...
| 文件 | 职责 |
|------|------|
| `apps/web/src/app/globals.css` | 所有 CSS 变量定义（两套主题） |
...
| 成功/已成交 | `--success-muted` | `#166534` | `--success-border` |
| 警告/待确认 | `#fffbeb` | `#92400e` | `#fde68a` |
